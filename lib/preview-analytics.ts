...
/** Heuristic: credit / loan style accounts (name-based; not from DB). */
...
/**
 * Cash-flow effect on the **primary** account for this row (matches paired
 * transfer legs in your sheet: in on one account, out on the other).
 */
...
export type MonthPreviewRollup = {
  ...
  /** Sum of signedPrimaryDelta for the month (transfers net across accounts). */
...
export function computePreviewAnalytics(
  ...
  const overall: PreviewOverall = {
    ...
    accountsTouched: Array.from(byAccountId.keys()).length,
  };

  return { overall, byAccount, byMonth: byMonthList };
}
